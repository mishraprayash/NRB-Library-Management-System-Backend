  ...
  try {
    ...
    borrowedBooksByMember.forEach((book) => {
      if (book.renewalCount >= MAX_RENEWAL_LIMIT) {
        invalidRenewalBooks.push({
          bookId: book.bookId,
          expiryDate: book.expiryDate,
          message: 'Max renewal limit reached',
          bookName: book.book.name,
        });
      } else {
        validRenewalBooks.push({
          bookId: book.bookId,
...
export const deleteSameMultipleBook = async (req, res) => {
  try {
    ...
    if (!bookExist.length) {
      return res
        .status(400)
        .json({ message: 'You cannot delete these books as they are assigned to someone.' });
    }
...
export const deleteBookByCount = async (req, res) => {
  try {
    ...
    if (stock > booksToDelete.length) {
      return res
        .status(400)
        .json({ message: `There are only ${booksToDelete.length} books available in the library` });
    }
...
export const addStock = async (req, res) => {
  try {
    ...
    const addedBook = Array.from({ length: stock }, () => ({
      bookCode,
      name: bookExists[0].name,
      authors: bookExists[0].authors,
      publisher: bookExists[0].publisher,
      publishedYear: bookExists[0].publishedYear,
      cost: bookExists[0].cost,
      pages: bookExists[0].pages,
      category: bookExists[0].category,
    }));
